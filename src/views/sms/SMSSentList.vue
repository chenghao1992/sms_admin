...
export default {
    ...
    methods: {
        ...
        handleQuery(isRequery) {
            ...
            if (this.formInline.submitTime) {
                queryObj.sendTimeStart = Moment(this.formInline.submitTime[0]).format('YYYY/MM/DD HH:mm:ss')
                queryObj.sendTimeEnd = Moment(this.formInline.submitTime[1]).format('YYYY/MM/DD HH:mm:ss')
            }
        ...
        handleBatchCancel: function(row) {
            ...
            this.$confirm('确认批量取消发送选中短信吗?', '提示', {}).then(() => {
                Api.undoSendSMS({ 
                    sendManagementId: this.multipleSelection.join(','), 
                }).then(res => {
                    if (res.code==0) {
                        this.$notify({
                            title: '成功',
