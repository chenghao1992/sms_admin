	...
	<div class="">
      ...
      <div class="signature" :key="item.id" v-for="(item, index) in form.signatures">
        <div class="signature-header">
          ...
          <span class="signature-remove" @click="removeItem(index,item)">x</span>
        </div>
...
import { inputBlurSearch,accurateSearch,modifyWeight,addUserInterfaces,userDetails,modifyUserInfo,usernameOnly } from '../../api/api';
export default {
  name: 'user-form',
  props: {
    id: {
    ...
    var validateInputSignature = (rule, value, callback) => {
     ...
     var validNumber = (rule, value, callback) => {
      ...
      preStatas:[],
      deleteAccountIds:'',
      nodefaultAccountIds:[], /*取消默认*/
      channnelDefaultStatus:false,
      preDefaultIdArr : [],
      deleteSigns:'',
      conmitChageWeight:false, /*确认修改权重*/
      rules2: {
        name: [
    ...
    handleSubmitChannel (channel) { /*修改权重*/
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          for(var i=0;i<this.form.signatures.length;i++){
            ...
            for(var j=0;j<this.form.signatures[i].channels.length;j++){
              ...
              if(this.form.signatures[i].channels[j].checked){
                if(this.preStatas.contains(this.form.signatures[i])){
                ...
                this.deleteAccountIds += this.form.signatures[i].channels[j].id + ','
              }
          ...
          if(this.deleteSigns)
            this.deleteSigns = this.deleteSigns.substring(0,this.deleteSigns.length - 1);
          if(weights)
          ...
          if(this.deleteAccountIds)
            this.deleteAccountIds = this.deleteAccountIds.substring(0,this.deleteAccountIds.length - 1);
          if(this.nodefaultAccountIds){
            this.nodefaultAccountIds = this.nodefaultAccountIds.join(',');
          }
          let paras = {
            ...
            deleteAccountIds:this.deleteAccountIds,
            nodefaultAccountIds:this.nodefaultAccountIds,
            userId:this.$route.params.id,
            signs:signs,
            weights:weights,
            deleteSigns:this.deleteSigns
          }
          modifyUserInfo(paras).then(res => {
            let {msg,code} = res;
            if(code==0){
              this.$message({
                message: msg,
    ...
    removeItem(index,item){  /*删除签名*/
      if(this.preStatas.contains(item)){
        /*item.channels.map((val,idx) => {
          if(val.checked){
            this.deleteAccountIds += val.id +','  
          }
        })*/
        this.deleteSigns += item.name+','
      }
      this.form.signatures.splice(index,1);  /*数组方法删除一条信息*/
    },
    setDefault(channel,index){   /*设置默认通道函数*/
      channel.filter((val,idx) => {
        val.defaultUse = index==idx?true:false;
      })
      if(this.preStatas.containsC(channel)){ /*判断是否是原生渲染的初始数据中操作*/
        channel.map((val,idx) => {
          if(!val.defaultUse){
            if(this.nodefaultAccountIds != []){
              if(!this.nodefaultAccountIds.contains(val.id)){
                this.nodefaultAccountIds.push(val.id);
              }
            }else{
              this.nodefaultAccountIds.push(val.id);
            }
      ...
      if(this.nodefaultAccountIds.contains(channel[index].id)){
        this.nodefaultAccountIds.removeByValue(channel[index].id)
      }
    },
    getDetails(){
      /*this.$route.params*/
      userDetails({userId:this.$route.params.id}).then(res => {
        let {data} = res;
        if(data.code == 0){
          if(data.result.user){
            this.form.name = data.result.user.username;
            this.prename = data.result.user.username;
            this.form.company_name = data.result.user.companyName;
            this.form.buniss_no = data.result.user.businessLicense;
            this.form.true_name = data.result.user.contactName;
            this.form.card = data.result.user.contactIdNumber;
            this.form.mobile = data.result.user.contactMobile;
          }
          if(data.result.flow){
            this.form.spacing = !data.result.flow.mobileInterval?'':data.result.flow.mobileInterval;
            this.form.minutMax = !data.result.flow.mobileTenMinTotal?'':data.result.flow.mobileTenMinTotal;
            this.form.hoursMax = !data.result.flow.mobileHourTotal?'':data.result.flow.mobileHourTotal;
            this.form.dayMax = !data.result.flow.mobileDailyTotal?'':data.result.flow.mobileDailyTotal;
          }
          if(data.result.accountsList){
            for(let i=0;i<data.result.accountsList.length;i++){
              var obj = {};
              obj.channels = [];
              obj.value = data.result.accountsList[i].sign;
              obj.name = data.result.accountsList[i].sign;
              for(let j=0;j<data.result.accountsList[i].list.length;j++){
                let singleObj = {};
                singleObj.id = data.result.accountsList[i].list[j].id;
                singleObj.name = data.result.accountsList[i].list[j].username;
                singleObj.channelName = data.result.accountsList[i].list[j].channelName;
                singleObj.weight = data.result.accountsList[i].list[j].weight;
                if(data.result.accountsList[i].list[j].isDefault == 0){
                  singleObj.defaultUse = true;
                  this.preDefaultIdArr.push(data.result.accountsList[i].list[j].id)
                }else{
                  singleObj.defaultUse = false;
                }
                if(data.result.accountsList[i].list[j].status == 2){
                   singleObj.checked = true; /*默认勾选*/
                }else{
                   singleObj.checked = false; /*默认勾选*/
                }
                if(data.result.accountsList[i].list[j].type == 1){
                  singleObj.defaultClass = 'spanDefaultNone';
                }else if(data.result.accountsList[i].list[j].type == 2){
                  singleObj.defaultClass = '';
                }
               
                singleObj.edit = false; 

                obj.channels.push(singleObj);
              }
              this.form.signatures.push(obj);
              this.preStatas.push(obj);
            }
            /*console.log(this.form.signatures);*/
          }
        }
      })
    }
...
Array.prototype.containsC = function (obj) {
  var i = this.length;
  while (i--) {
    if (this[i].channels === obj) {
      return true;
    }
  }
  return false;
