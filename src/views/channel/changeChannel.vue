    ...
    <div class="changeChannel">
        <el-form :model="editForm" label-width="120px" style="width:60%" label-position="left" :rules="editFormRules" ref="editForm">
            ...
            <el-form-item label="通道识别码" prop="channelIdDesc">
                <el-input v-model="editForm.channelIdDesc" auto-complete="off" :disabled="noreset" placeholder="请输入通道识别码"></el-input>
            </el-form-item>
...
<script>
import { saveChannel,checkChannelDetails } from '../../api/api';
...
export default {
    ...
    mounted(){
    	checkChannelDetails({channelId:this.$route.params.channelId}).then( res => {
			let {data} = res;
			if(data.code == 0){
				this.editForm.type = data.result.type+'';
				this.editForm.channelIdDesc = data.result.channelIdDesc;
				this.editForm.channelName = data.result.channelName;
				this.editForm.longSms = data.result.longSms;
				this.editForm.batchSms = data.result.batchSms;
				this.editForm.separatorSipl = data.result.separatorSipl;
				this.editForm.apiUrl = data.result.apiUrl;
				this.editForm.id = this.$route.params.channelId;
			}
    	})
    }
}
